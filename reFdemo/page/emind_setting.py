...
from page.shard import gradio, chatbot_tab_visible, custom_tab_visible,default_settings,settings
from page.logger import create_logging
...
def select_tokenizer(tokenizer):
	...
	logger.info("tokiner已经配置,选项为{}".format(tokenizer))
...
def save_model_speech_rec():
	...
	if text_audio_model_default_conf != text_audio_model_conf:
		agent._update_pipes("Text2AudioDima",text_audio_model_conf)#更新模型:更改参数
		logger.info("声音识别模型参数已经更新")
	else:
		logger.info("声音识别模型参数没有改变")
def save_model_search():
	...
	if spotify_default_conf != spotify_conf:
		agent._update_pipes("spotify",spotify_conf)#更新模型:更改参数
...
def save_model_music_rec():
	...
	if music_recomandation_spotify_default_conf != music_recomandation_spotify_conf:
		agent._delete_pipes("music_recomandation_spotify",music_recomandation_spotify_conf)#更新模型:更改参数
	...
	with gr.Tab("自定义配置",elem_id="custom_config",visible=False) as Argument_tab:
		...
		with gr.Tab(label="音乐生成",elem_id="music_roc") as Music_roc_tab:
			...
			with gr.Row(elem_id="model_paramters"):
				...
				gradio["debug"]=  gr.Radio(choices=["store_true","store_false"],label="调试选项",value="store_true",interactive=True)#调试选项
				gradio["sentiment"]=  gr.Radio(choices=["store_true","store_false"],label="情绪基调",value="store_true",interactive=True)
			...
			with gr.Row(elem_id="save_model",equal_height=False):
				gradio["music_gen_save_button"]= gr.Button(save_icon,onclick=save_model_music_gen(),elem_id="save_model",label="保存",placeholder="保存配置")
    
		with gr.Tab(label="声音识别"):
			gradio["text_audio_model"]  = gr.Radio(choices=["dima806/music_genres_classification","lewtun/distilhubert-finetuned-music-genres"],
            label="模型选择")
			...
			with gr.Row():
				gradio["speech_rec_save_button"]= gr.Button(save_icon,onclick=save_model_speech_rec,elem_id="save_model",label="保存",placeholder="保存配置")
		...
		with gr.Tab(label="联网搜索"):#spotify,google search
			...
			with gr.Row():
				gradio["internet_save_model"]= gr.Button(save_icon,onclick=save_model_search,elem_id="save_model",label="保存",placeholder="保存配置")
    
		with gr.Tab(label="人声分离"):
			with gr.Row():#model
				gradio["source_sperate_model"]=gr.Radio(choices=["htdemucs","htdemucs_ft","hdemucs_mmi","mdx"],value="htdemucs",label="music_type",interactive=True)
			...
			with gr.Row():
				gradio["split_save_model"] = gr.Button(save_icon,onclick=save_model_split,elem_id="save_model",label="保存",placeholder="保存配置")
		...
		with gr.Tab(label="音乐推荐"):#TODO:
			gradio["music_recomend_genres"]=gr.Textbox(value="pop", type="text" ,lines=1,label="音乐类型",interactive=True)
			...
			with gr.Row():
				gradio["music_recommend_save_model"]= gr.Button(save_icon,onclick=save_model_music_rec,elem_id="save_model",label="保存",placeholder="保存配置")
#事件监听，调用处理函数
...
def create_paramters_envent_hander():
	...
	gradio["debug"].select(select_debug_options,[gradio["debug"]])
	gradio["sentiment"].select(select_emotion_options,[gradio["sentiment"]])
	gradio["batch_size"].change(get_batch_size,[gradio["batch_size"]])
	gradio["tokenizer"].select(select_tokenizer,[gradio["tokenizer"]])
	#下面是另一个音乐模型
	...
	#声音识别
	gradio["text_audio_model"].select(select_text_audio_model,[gradio["text_audio_model"]])
	...
	#联网搜索
	gradio["spotify_base_url"].change(get_spotify_base_url,[gradio["spotify_base_url"]])
	gradio["spotify_key"].change(get_spotify_key,[gradio["spotify_key"]])
	gradio["search_options"].select(select_search_options,[gradio["search_options"]])
	gradio["google_base_url"].change(get_google_base_url,[gradio["google_base_url"]])
	gradio["google_key"].change(get_google_key,[gradio["google_key"]])
	gradio["google_eninger"].change(get_google_eninger,[gradio["google_eninger"]])
	#人声分离
	gradio["source_sperate_model"].select(select_source_sperate_model,[gradio["source_sperate_model"]])
	gradio["save_file_type"].select(select_save_file_type,[gradio["save_file_type"]])
	...
	#音乐推荐
	gradio["music_recomend_genres"].change(get_music_recomend_genres,[gradio["music_recomend_genres"]])
	gradio["music_recommend_model_with_chinese"].select(select_music_recommend_model_with_chinese,[gradio["music_recommend_model_with_chinese"]])
	gradio["music_recommend_model_with_english"].select(select_music_recommend_model_with_english,[gradio["music_recommend_model_with_english"]])
